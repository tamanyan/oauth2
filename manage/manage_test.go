...
func TestManager(t *testing.T) {
	Convey("Manager test", t, func() {
		...
		manager.MustTokenStorage(store.NewTokenStore(store.NewTokenConfig(":memory:", "sqlite3", "access_token"), 600))

		clientStore, _ := store.NewClientStore(store.NewClientConfig(":memory:", "sqlite3", "client"))
